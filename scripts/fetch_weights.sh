#!/usr/bin/env bash
# fetch_weights.sh — populate the state home with the Bonsai notarized model weights.
#
# The composed notary keeps weights OUTSIDE every repo, under $BONSAI_NOTARY_HOME/models
# (default ~/.local/trinote/models) — the same shared home the engine and chain_c use. This is
# the canonical install step that puts them there. Two weight artifacts:
...
#
# GGUF resolution (first that verifies wins):
#   1. already present + sha256-verified in the state home          -> skip
#   2. a verified LOCAL copy under $BONSAI_WEIGHTS_REPO / --from DIR -> link/copy (OPT-IN; UNSET by
#      default, so the default path DOWNLOADS from HuggingFace, never a local checkout)
#   3. otherwise download from HuggingFace ($HF_REPO) into the state home.
# The safetensors is then (re)built from the verified GGUF by the import step (engine venv required).
#
# Each file is checksum-verified (sha256) against the provenance hashes recorded in
# artifacts/atlas-notarized-bonsai-8b.identity.json.
...
# Weights live under $BONSAI_NOTARY_HOME (default ~/.local/trinote/models), NEVER inside the repo tree.
NOTARY_HOME="${BONSAI_NOTARY_HOME:-$HOME/.local/trinote}"
MODELS_DIR="${BONSAI_MODELS_DIR:-$NOTARY_HOME/models}"
# OPT-IN local source to reuse (link/copy) the GGUF instead of downloading. UNSET by default, so the
# default path downloads the GGUF from HuggingFace (never a local directory). Set $BONSAI_WEIGHTS_REPO
# or pass --from DIR to opt back in. Only a source for the state home; never used as a runtime path.
WEIGHTS_REPO="${BONSAI_WEIGHTS_REPO:-}"
LOCAL_SRCS=()
LINK=1   # default: symlink a local copy into the state home; --copy makes a real copy
...
Options:
  -f, --force            Re-fetch even if a verified file already exists in the state home.
      --copy             Copy a local source into the state home (default: symlink it).
      --link             Symlink a local source into the state home (default).
      --from DIR         Add a local directory to search for a verified copy to reuse.
      ...
      --dry-run          Print what would be fetched; do nothing.
  -h, --help             Show this help and exit (no side effects).

Environment variables:
  BONSAI_NOTARY_HOME State/secrets home; weights land in \$BONSAI_NOTARY_HOME/models (default ~/.local/trinote)
  BONSAI_WEIGHTS_REPO OPT-IN local checkout to reuse the GGUF (default: unset — download from HF)
...
while (($#)); do
  case "$1" in
    ...
    --link)
      ...
      ;;
    --from)
      [[ -n "${2:-}" ]] || { echo "${SCRIPT_NAME}: --from requires a DIR" >&2; exit 2; }
      LOCAL_SRCS+=("$2")
      shift 2
      ;;
    --)
...
# Local source dirs to reuse (link/copy) before downloading: any --from DIRs first, then the
# split layout of a bonsai-notarized-bitnet checkout (its models/ and artifacts/model/).
if [[ -n "$WEIGHTS_REPO" ]]; then
  LOCAL_SRCS+=("$WEIGHTS_REPO/models" "$WEIGHTS_REPO/artifacts/model" "$WEIGHTS_REPO")
fi
...
# Fetch one logical artifact into the state home: relpath, expected sha256, remote filename.
fetch_artifact() {
  ...
  if ((DRY_RUN)); then
    log "[dry-run] would ensure ${dest} (reuse a local copy if verified, else fetch ${url})"
    return 0
  fi

  # 1. Already present + verified in the state home? skip.
  if ((FORCE == 0)) && [[ -e "$dest" ]] && verify_checksum "$dest" "$expected"; then
    log "already in state home, verified: ${fname} (skip; --force to refetch)"
    return 0
  fi

  mkdir -p "$(dirname "$dest")"

  # 2. Reuse a verified LOCAL copy by linking (or copying) it into the state home — no re-download,
  #    no multi-GB duplication.
  if ((FORCE == 0)); then
    for src in "${LOCAL_SRCS[@]}"; do
      local cand="${src}/${fname}"
      if [[ -e "$cand" ]] && verify_checksum "$cand" "$expected"; then
        if ((LINK)); then
          ln -sfn "$(readlink -f "$cand")" "$dest"
          log "linked local copy into state home: ${cand} -> ${dest}"
        else
          cp -f "$cand" "$dest"
          log "copied local copy into state home: ${cand} -> ${dest}"
        fi
        return 0
      fi
    done
  ...
  # 3. Download from HuggingFace into the state home.
  download "$url" "$dest"
  if ! verify_checksum "$dest" "$expected"; then
    die "downloaded ${fname} failed checksum verification (file left at ${dest} for inspection)"
  fi
  log "fetched into state home: ${fname}"
}
...
# from the GGUF on a given host. Requires the engine venv (numpy/safetensors).
import_safetensors() {
  ...
  if ((FORCE == 0)) && [[ -e "$dest" ]] && verify_checksum "$dest" "$SAFETENSORS_SHA256"; then
    log "already in state home, verified: ${fname} (skip; --force to rebuild)"
    return 0
  fi
  [[ -e "$gguf" ]] || die "cannot build ${fname}: GGUF missing at ${gguf} (fetch the GGUF first)"

  local engine="${BONSAI_ENGINE_DIR:-$ROOT/engine}"
  [[ -d "$engine/bonsai/src" ]] || die "engine not found at ${engine} (set \$BONSAI_ENGINE_DIR); needed to import the GGUF"
  local py="$engine/bonsai/.venv/bin/python"; [[ -x "$py" ]] || py="python3"
  log "importing GGUF -> ${fname} (deterministic creation step; engine: ${engine})"
  mkdir -p "$(dirname "$dest")"
  BONSAI_NOTARY_HOME="$NOTARY_HOME" PYTHONPATH="$engine/bonsai/src" "$py" \
    -m trinote.cli.import_bonsai_gguf_cli --gguf "$gguf" --out "$dest" \
    || die "GGUF -> safetensors import failed (need the engine venv: ${engine}/bonsai/.venv)"
  verify_checksum "$dest" "$SAFETENSORS_SHA256" \
    || die "imported ${fname} failed checksum verification (left at ${dest} for inspection)"
  log "built (imported) into state home: ${fname}"
}
